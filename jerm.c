...
#define A 6378137.0 /* 赤道半径 m */
#define F 298.257222101 /* 扁平率の逆数=1/((A-B)/A) */
#define B 6356752.31414035584785210687 /* 極半径=A-A/F */
...
xymove(double nt, double et, double *delta_n, double *delta_e)
{
	...
	cos_nt = cos(nt);
	sin_nt = sin(nt);
	r1 = sqrt(A * A * cos_nt * cos_nt + B * B * sin_nt * sin_nt); /* 北緯 nt の地球半径 */
	r2 = A * cos_nt; /* 北緯 nt の円の半径 */
	rdn = (nt - nt0); /* 北緯変位 [rad] */
	rde = (et - et0); /* 東経変位 [rad] */
	*delta_n = r1 * rdn; /* 北変位[m] */
	*delta_e = r2 * rde; /* 東変位[m] */
}
...
main(int argc, char *argv[])
{
	...
	int daemon = 0;
	myname = argv[0];
	while ((ch = getopt(argc, argv, "46b:d:Df:Fijl:p:P:r:s:TVxze")) != EOF) {
		switch (ch) {
			int x;
		default:
		case 'V':
